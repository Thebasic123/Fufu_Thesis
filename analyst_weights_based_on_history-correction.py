    ...
    try:
        ...
        for line in open(filename, 'r'):
            ...
            if line != '':
                ...
                if action == 'L':
                    performance = float(performance) - 1.0
                elif action == 'S':
                    performance = 1.0 - float(performance)
                if analyst in analyst_score:
                    analyst_score[analyst] += performance
                else:
                    analyst_score[analyst] = performance
...
for i in sorted(analyst_score.keys(), key = lambda x: analyst_score[x]):
   print(i, analyst_score[i])
